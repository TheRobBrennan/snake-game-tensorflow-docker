...
// Set canvas dimensions - important to set this explicitly for Safari
...
function draw() {
  // Clear the entire canvas with a single operation
  ...
  for (let i = 0; i < snake.length; i++) {
    ...
    const x = Math.floor(segment.x * scale);
    const y = Math.floor(segment.y * scale);
    ctx.fillRect(x, y, scale, scale);
  }

  // Draw fruit
  ctx.fillStyle = 'red';
  const fruitX = Math.floor(fruit.x * scale);
  const fruitY = Math.floor(fruit.y * scale);
